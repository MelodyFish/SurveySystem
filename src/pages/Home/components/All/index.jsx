...
export default function Mine(props) {
  ...
  const columns = [
    {
      ...
      dataIndex: 'title',
      key: 'title',
      render: (_, {surveyId, title, creatorCn, createTime, sstatus, startTime, endTime, subTitle, creator})=> {
        return (
          <a 
            style={{cursor: 'pointer'}}
            onClick={()=>redirectSetting(surveyId, title, creatorCn, createTime, sstatus, startTime, endTime, subTitle, creator)}
          >
            {title}
          </a>
        )
  ...
  function redirectSetting(surveyId, title, creatorCn, createTime, sstatus, startTime, endTime, subTitle, creator ) {
    if(username===creator) {
      nav('/setting', {state: {surveyId, title, creatorCn, createTime, sstatus, startTime, endTime, subTitle, creator}})
    } else {
      message.error('不能进入其他用户的问卷设置!')
    }
